                  ...
                  {{--  <td class="text-center">
                        ...
                        <div class="btn-group" role="group">
                            ...
                            <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                <form method="get" action="{{route('mostrarActor', $pelicula->film_id)}}">
                                    @csrf
                                    <button class="dropdown-item btn btn-light">Editar</button>
                                </form>
                                <a class="dropdown-item btn btn-light" href="{{route('eliminarActor',$pelicula->film_id)}}">Eliminar</a>
                             --}}{{--   <form method="POST" action="{{route('eliminarActor', $actor->actor_id)}}">
                                    {{ @method_field('DELETE') }}
                                    @csrf
                                    <button class="dropdown-item btn btn-light">Eliminar</button>
                                </form>--}}{{--
                                <a class="dropdown-item btn btn-light" href="{{route('actorPelicula', $pelicula->film_id)}}">Ver peliculas</a>
